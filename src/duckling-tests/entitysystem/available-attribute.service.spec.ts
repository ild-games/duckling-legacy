...
describe("AvailableAttributeService", function() {
    beforeEach(function() {
        ...
        this.available = new AvailableAttributeService(
            attributeDefault,
            projectService
        );
    });
    ...
    it("called with no entity returns the available attributes", function() {
        expect(sort(this.available.availableAttributes())).to.eql(
            sort([POS, COL, DRAW, CUSTOM_ONE, CUSTOM_TWO])
        );
    });
    ...
    it("called with a partial entity returns the availble attributes", function() {
        ...
        entity[POS] = {};
        entity[CUSTOM_ONE] = {};
        expect(sort(this.available.availableAttributes(entity))).to.eql(
            sort([COL, DRAW, CUSTOM_TWO])
        );
    });
