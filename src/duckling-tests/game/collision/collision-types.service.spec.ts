...
import {
  ...
  NONE_COLLISION_TYPE,
} from '../../../duckling/game/collision/collision-types.service';
import {
  COLLISION_KEY,
  defaultCollision,
} from '../../../duckling/game/collision/collision-attribute';
...
describe('CollisionTypesService', function () {
  ...
  it('puts new collision types in the store', function () {
    ...
    expect(
      Array.from(
        this.storeService.state.getValue().collision.collisionTypes.values()
      )
    ).to.contain('collision-type1');
  });

  it('is aware of new collision types added', function () {
    this.collisionTypesService.addCollisionType('collision-type1');
    expect(
      Array.from(this.collisionTypesService.collisionTypes.getValue().values())
    ).to.contain('collision-type1');
  });
  ...
  it("retrieve a collision type of 'none' on an entity that has the default collision", function () {
    ...
    expect(this.collisionTypesService.getCollisionTypeForEntity(entity)).to.eql(
      NONE_COLLISION_TYPE
    );
  });
  ...
  it("registers only the 'none' collision type when given stream of collision types that does not contain none", function () {
    (this.collisionTypesService as any)._registerAnconaCollisionTypes({
      collisionTypes: ['custom-type'],
    });
    expect(
      Array.from(this.collisionTypesService.collisionTypes.getValue().values())
    ).to.contain('none');
  ...
  it('registers custom collision types when given json of collision types that does not contain none', function () {
    (this.collisionTypesService as any)._registerAnconaCollisionTypes({
      collisionTypes: ['custom-type1', 'custom-type2'],
    });
    expect(
      Array.from(this.collisionTypesService.collisionTypes.getValue().values())
    )
      .to.contain('custom-type1')
      .and.to.contain('custom-type2');
  });

  it('can register multiple collision types at once', function () {
    (this.collisionTypesService as any)._registerCollisionTypes([
      'custom-type1',
      'custom-type2',
    ]);
    expect(
  ...
  it('can find the unknown collision types from a given map', function () {
    ...
    let unknownCollisionTypes = (
      this.collisionTypesService as any
    )._findUnknownCollisionTypes(map);
  ...
  it('can get the unique collision types in a given map', function () {
    ...
    let uniqueCollisionTypes = (
      this.collisionTypesService as any
    )._getUniqueCollisionTypesInRawMapFile(map);
