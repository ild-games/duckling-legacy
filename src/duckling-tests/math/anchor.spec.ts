...
describe("positionToAnchor", function() {
    ...
    it("Can calculate a positive position", function() {
        expect(positionToAnchor({x: 5, y: -10}, vec)).to.eql({x: -0.5, y: 2});
    });

    it("Handles a zero width", function() {
        expect(positionToAnchor({x: 10, y: 5}, {x: 5, y: 0})).to.eql({x: -2, y: 0});
    });

    it("Handles a zero height", function() {
        expect(positionToAnchor({x: 10, y: 5}, {x: 0, y: 5})).to.eql({x: 0, y: -1});
    });
});
