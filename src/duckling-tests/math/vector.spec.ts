...
describe("vectorAdd", function() {
    it("{1, 1} + {1, 1} = {2, 2}", function() {
        expect(vectorAdd({ x: 1, y: 1 }, { x: 1, y: 1 })).to.eql({
            x: 2,
            y: 2,
        });
    });
    ...
    it("{0, 0} + {-1, -1} = {-1, -1}", function() {
        expect(vectorAdd({ x: 0, y: 0 }, { x: -1, y: -1 })).to.eql({
            x: -1,
            y: -1,
        });
    });
    ...
    it("{0, 0} + {0, 0} = {0, 0}", function() {
        expect(vectorAdd({ x: 0, y: 0 }, { x: 0, y: 0 })).to.eql({
            x: 0,
            y: 0,
        });
    });
...
describe("vectorSubtract", function() {
    it("{1, 1} - {1, 1} = {0, 0}", function() {
        expect(vectorSubtract({ x: 1, y: 1 }, { x: 1, y: 1 })).to.eql({
            x: 0,
            y: 0,
        });
    });
    ...
    it("{0, 0} - {-1, -1} = {1, 1}", function() {
        expect(vectorSubtract({ x: 0, y: 0 }, { x: -1, y: -1 })).to.eql({
            x: 1,
            y: 1,
        });
    });
    ...
    it("{0, 0} - {0, 0} = {0, 0}", function() {
        expect(vectorSubtract({ x: 0, y: 0 }, { x: 0, y: 0 })).to.eql({
            x: 0,
            y: 0,
        });
    });
...
describe("vectorRotate", function() {
    ...
    it("takes radians and rotates clockwise", function() {
        ...
        expect(result.x).to.be.within(0.909, 0.91);
