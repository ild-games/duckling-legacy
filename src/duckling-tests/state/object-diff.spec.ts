...
describe("changeType", function() {
    it("returns Equal for two equivalent primitives", function() {
        expect(changeType("foo", "foo")).toBe(ChangeType.Equal);
    });

    it("returns PrimitiveChange for two primitives of the same type", function() {
        expect(changeType("foo", "bar")).toBe(ChangeType.PrimitiveChange);
    });

    it("returns PrimitiveChange for two primitives of different types", function() {
        expect(changeType("foo", 3)).toBe(ChangeType.PrimitiveChange);
    });

    it("returns PrimitiveChange for a primitive and null", function() {
        expect(changeType(null, 3)).toBe(ChangeType.PrimitiveChange);
    });

    it("returns ComplexChange for a primitive and an object", function() {
        expect(changeType("foo", {foo: 3})).toBe(ChangeType.ComplexChange);
    });

    it("returns ComplexChange for a null and an object", function() {
        expect(changeType(null, {foo: 3})).toBe(ChangeType.ComplexChange);
    });

    it("returns Equal for two empty objects", function() {
        expect(changeType({}, {})).toBe(ChangeType.Equal);
    });

    it("returns Equal for two flat objects", function() {
        expect(changeType({foo: 3, bar: 2}, {foo:3, bar: 2})).toBe(ChangeType.Equal);
    });

    it("returns PrimitiveChange for two flat objects with one differing key", function() {
        expect(changeType({foo: 3, bar: 4}, {foo:3, bar: 2})).toBe(ChangeType.PrimitiveChange);
    });
    ...
    it("returns PrimitiveChange for two flat objects with an extra key", function() {
        expect(changeType({foo: 3, bar: 2, spam: 2}, {foo:3, bar: 2})).toBe(ChangeType.PrimitiveChange);
    });

    it("returns Equal for nested object", function() {
        expect(changeType({foo: 3, bar : {cake : 1}}, {foo: 3, bar : {cake : 1}})).toBe(ChangeType.Equal);
    });

    it("returns PrimitiveChange for a nested prinmitive change", function() {
        expect(changeType({foo: 3, bar : {cake : 1}}, {foo: 3, bar : {cake : 3}})).toBe(ChangeType.PrimitiveChange);
    });

    it("returns PrimitiveChange for a nested missing key", function() {
        expect(changeType({foo: 3, bar : {cake : 1}}, {foo: 3, bar : {}})).toBe(ChangeType.PrimitiveChange);
    });

    it("returns ComplexChange for a nested complex change", function() {
        expect(changeType({foo: 3, bar : {cake : 1}}, {foo: 3, bar : {train : 2}})).toBe(ChangeType.ComplexChange);
    });
});
