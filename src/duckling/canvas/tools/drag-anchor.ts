...
export function drawAnchor(
  ...
  let sprite = new Sprite(
    new Texture(
      assetService.get({ key: "anchor_arrow", type: "TexturePNG" }, true)
    )
  );
...
export function anchorContainsPoint(
  box: Box2,
  anchor: DragAnchor,
  point: Vector,
  canvasScale: number
  ...
  let position = getAnchorPosition(box, anchor);
  let relativeLocation = vectorRotate(
    vectorSubtract(point, position),
    -anchor.rotation
  );
  ...
  return (
    ...
    relativeLocation.x <= xSize &&
    -ySize <= relativeLocation.y &&
    relativeLocation.y <= ySize
  );
}

export function getResizeFromDrag(
  box: Box2,
  anchor: DragAnchor,
  ...
  return {
    ...
    position: {
      x:
        box.position.x +
        xPositionMultiplier * (endPosition.x - startPosition.x),
      y:
        box.position.y + yPositionMultiplier * (endPosition.y - startPosition.y)
    },
    dimension: {
      x:
        box.dimension.x +
        xDimensionMultiplier * (endPosition.x - startPosition.x),
      y:
        box.dimension.y +
        yDimensionMultiplier * (endPosition.y - startPosition.y)
    }
  };
}
