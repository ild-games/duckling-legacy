 ...
 */
export function minCornerSnapDistance(
    movementDestinationWithoutSnap: Vector,
    box: Box2,
    gridSize: Vector
): Vector {
    return {
        x: _getDistance(
            movementDestinationWithoutSnap.x,
            box.dimension.x,
            gridSize.x / SNAP_FRACTION
        ),
        y: _getDistance(
            movementDestinationWithoutSnap.y,
            box.dimension.y,
            gridSize.y / SNAP_FRACTION
        ),
    };
}
...
export function _getDistance(
    ...
    let leftSnapDistance =
        roundToMultiple(leftPosition, gridSize) - leftPosition;
    let rightSnapDistance =
        roundToMultiple(rightPosition, gridSize) - rightPosition;
