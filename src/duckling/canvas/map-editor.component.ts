...
export class MapEditorComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  private _saveMetaData() {
    let scrollLeft = (this.canvasElement as any).canvasContainerDiv
      .nativeElement.parentElement.scrollLeft;
    let scrollTop = (this.canvasElement as any).canvasContainerDiv.nativeElement
  ...
  private _loadMapMetaData() {
    ...
    this.initialScrollPosition = {
      x: curMapUserMetaData.scrollLeft,
      y: curMapUserMetaData.scrollTop
    };
  ...
  private _redrawAllDisplayObjects() {
    if (this._drawingCache.layers.length === 0) {
      let drawnConstructs: DrawnConstruct[] = [
        ...
        ...this._entityDrawerService.drawEntitySystem(
  ...
  private _buildCanvasBackground(): DrawnConstruct {
    let drawnConstruct = new CanvasBackgroundDrawnConstruct(
      this.projectService.project.value.currentMap.dimension
    );
  ...
  private _buildGrid(): DrawnConstruct {
    ...
    let drawnConstruct = new GridDrawnConstruct(
      this.scale,
      this.projectService.project.value.currentMap.dimension,
      this.projectService.project.value.currentMap.gridSize
    );
...
class CanvasBackgroundDrawnConstruct extends DrawnConstruct {
  ...
  constructor(private _dimension: Vector) {
    super();
    drawCanvasBackground({ x: 0, y: 0 }, this._dimension, this._graphics);
  }
...
class GridDrawnConstruct extends DrawnConstruct {
  ...
  constructor(
    ...
    drawCanvasBorder({ x: 0, y: 0 }, this._dimension, this._graphics);
  }
