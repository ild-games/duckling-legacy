...
export class MapEditorComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  ngOnInit() {
    this._drawerServiceSubscription =
      this._entityDrawerService.redraw.subscribe((entityCacheValid) => {
  ...
  ngAfterViewInit() {
    this._redrawInterval = timer(0, 1000 / this._framesPerSecond).subscribe(
      () => this._drawFrame()
    ) as Subscriber<any>;
  }
  ...
  private _loadMapMetaData() {
    let curMapUserMetaData =
      this.projectService.project.value.userMetaData.mapMetaData[
        this.projectService.project.value.currentMap.key
      ];
  ...
  private _redrawAllDisplayObjects() {
    if (this._drawingCache.layers.length === 0) {
      ...
      let layeredDrawnConstructs =
        this._renderPriorityService.sortDrawnConstructs(drawnConstructs);
  ...
  private _setTool(tool: BaseTool) {
    ...
    this._toolDrawnConstructChangedSubscription =
      this.tool.drawnConstructChanged.subscribe(() => {
