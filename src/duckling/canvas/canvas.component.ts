...
export class CanvasComponent implements OnChanges, OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    if (
      this.initialScrollPosition.x === 0 &&
      this.initialScrollPosition.y === 0
    ) {
      this._centerStage();
    } else {
  ...
  setupContainingElementEvents() {
    ...
    this.canvasContainerDiv.nativeElement.parentElement.onwheel = (
      event: WheelEvent
    ) => this.onScroll(event);
    this.canvasContainerDiv.nativeElement.parentElement.onmousemove = (
      event: MouseEvent
    ) => event.preventDefault();
    this.canvasContainerDiv.nativeElement.parentElement.onkeyup = (
      event: KeyboardEvent
    ) => this.onKeyUp(event);
    this.canvasContainerDiv.nativeElement.parentElement.onkeydown = (
      event: KeyboardEvent
    ) => this.onKeyDown(event);
  }

  ngOnChanges(changes: {
    stageDimensions?: SimpleChange;
    scale?: SimpleChange;
    initialScrollPosition?: SimpleChange;
  }) {
    ...
    if (changes.scale) {
      this.onScaleChanged(
        changes.scale.previousValue,
        changes.scale.currentValue
      );
    }
  ...
  onScaleChanged(oldScale: number, newScale: number) {
    ...
    let zoomPoint = {
      x:
        (this._zoomInCanvasCoords.x - stagePosition.x) / oldScale +
        this.stageDimensions.x / 2,
      y:
        (this._zoomInCanvasCoords.y - stagePosition.y) / oldScale +
        this.stageDimensions.y / 2
    };
  ...
  onPaste(event: ClipboardEvent) {
    this.elementPaste.emit(
      this._stageCoordsFromCanvasCoords(this._mouseLocation)
    );
  }
  ...
  forwardContainingDivMouseEvent(event: MouseEvent) {
    this.canvasRoot.nativeElement.dispatchEvent(
  ...
  forwardContainingDivWheelEvent(event: WheelEvent) {
    this.canvasRoot.nativeElement.dispatchEvent(
      new WheelEvent(event.type, event)
    );
  }

  scrollTo(scrollToCoords: Vector) {
    this.canvasContainerDiv.nativeElement.parentElement.scrollLeft =
      scrollToCoords.x;
    this.canvasContainerDiv.nativeElement.parentElement.scrollTop =
      scrollToCoords.y;
  }
  ...
  private _resizeCanvasElements() {
    ...
    this.scrollerDimensions.x =
      this.elementDimensions.x * 2 +
      this.stageDimensions.x * this.scale -
      this._scrollStageOffset * 2;
    this.scrollerDimensions.y =
      this.elementDimensions.y * 2 +
      this.stageDimensions.y * this.scale -
      this._scrollStageOffset * 2;
    if (this._renderer) {
  ...
  private _stageCoordsFromCanvasCoords(canvasCoords: Vector): Vector {
    ...
    return {
      x:
        (canvasCoords.x - position.x) / this.scale + this.stageDimensions.x / 2,
  ...
  private _render() {
    if (this._renderer) {
      ...
      this.canvasDisplayObject.position.x =
        position.x + 0.5 - (this.stageDimensions.x * this.scale) / 2;
      this.canvasDisplayObject.position.y =
        position.y + 0.5 - (this.stageDimensions.y * this.scale) / 2;
   ...
   */
  get _stagePosition() {
    let canvasScrollDifferenceWidth =
      this.scrollerDimensions.x - this.elementDimensions.x;
    let canvasScrollDifferenceHeight =
      this.scrollerDimensions.y - this.elementDimensions.y;
    return {
      x:
        this.elementDimensions.x / 2 +
        canvasScrollDifferenceWidth / 2 -
        this.scrollPosition.x,
      y:
        this.elementDimensions.y / 2 +
        canvasScrollDifferenceHeight / 2 -
