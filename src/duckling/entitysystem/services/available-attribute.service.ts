...
export class AvailableAttributeService {
  ...
  availableAttributes(entity: Entity = {}) {
    return this._availableDefaultAttributes(entity).concat(
      this._availableCustomAttributes(entity)
    );
  }
  ...
  private _availableCustomAttributes(entity: Entity): AttributeKey[] {
    let available: AttributeKey[] = [];
    for (let customAttribute of this._projectService.project.getValue()
      .customAttributes) {
