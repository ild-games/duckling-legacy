...
import { MapParserService, createRawMap } from './map-parser.service';
import {
  switchProjectAction,
...
export class ProjectService {
  ...
  private async _loadCustomAttributes() {
    let attributes = await fs_api.glob(`${this._customAttributesRoot}/*.json`);
    let attributePromises: Promise<void>[] = [];
    for (let customAttribute of attributes) {
      attributePromises.push(
        this._jsonLoader.getJsonFromPath(customAttribute).then((json) => {
  ...
  getMaps(): Promise<string[]> {
    ...
    return fs_api
      .glob(`${mapsRoot}/**/*.map`)
      .then((maps) => maps.map((map) => this._mapPathToRoot(mapsRoot, map)));
  }
  ...
  private _addMigrationToVersionInfo(migrationName: string, options?: any) {
    let newVersionInfo =
      this._migrationService.updateVersionInfoWithExistingCodeMigration(
        this.project.value.versionInfo,
        migrationName,
        options
      );
