...
export const drawableBoundingBox: AttributeBoundingBox<DrawableAttribute> = {
    ...
    setBox(
        ...
        if (drawableAttribute.topDrawable) {
            return {
                ...drawableAttribute,
                topDrawable: setDrawableBox(
                    drawableAttribute.topDrawable,
                    resizeToBox,
                    assetService
                ),
            };
...
function setContainerBox(
    drawable: ContainerDrawable,
    resizeToBox: Box2,
    assetService: AssetService
): ContainerDrawable {
    let currentBox = getDrawableBox(drawable, assetService);
    let currentPosition = anchorToPosition(
        drawable.anchor,
    ...
    return {
        ...
        drawables: drawable.drawables.map((childDrawable) => {
            ...
            return setDrawableBox(
                childDrawable,
                resize(currentBox, resizeToBox, childBox, newPosition),
                assetService
            );
        }),
    };
...
function setImageBox(
    ...
    if (drawable.isTiled) {
        return {
            ...drawable,
            anchor: positionToAnchor(
                resizeToBox.position,
                resizeToBox.dimension
            ),
...
function setBoxViaScale<T extends Drawable>(
    drawable: T,
    resizeToBox: Box2,
    assetService: AssetService
): T {
    ...
    return {
        ...
        scale: {
            x:
                (resizeToBox.dimension.x / originalSize.dimension.x) *
                drawable.scale.x,
            y:
                (resizeToBox.dimension.y / originalSize.dimension.y) *
                drawable.scale.y,
        },
...
function setTileBlockBox(
    drawable: TileBlockDrawable,
    resizeToBox: Box2,
    assetService: AssetService
): TileBlockDrawable {
