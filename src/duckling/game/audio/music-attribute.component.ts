...
export class MusicAttributeComponent {
    ...
    onMusicChanged(event: Music, index: number) {
        ...
        this.attributeChanged.emit(immutableAssign(this.attribute, {musics: musicsPatch}));
    }

    onMusicsChanged(newMusics : Music[]) {
        this.attributeChanged.emit(immutableAssign(this.attribute, {musics: newMusics}));
    }
}
