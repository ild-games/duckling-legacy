...
@Component({
  ...
  template: `
    <div *ngIf="selections?.length === 0">
      <mat-card>
        <mat-card-title> No Single Entity Selected </mat-card-title>
      </mat-card>
    </div>
...
export class EntityEditorComponent {
  ...
  onEntityChanged(entity: Entity, mergeKey: MergeKey = newMergeKey()) {
  ...
  addAttribute(key: AttributeKey) {
    ...
    if (this._projectService.isCustomAttribute(key)) {
      defaultAttribute = getDefaultCustomAttributeValue(
        this._projectService.getCustomAttribute(key)
      );
    } else {
    ...
    patch = {};
    patch[key] = this._attributeDefaultAugmentation.augmentAttribute(key, {
      entity: this.selections[0].entity,
      key: this.selections[0].key,
    });
