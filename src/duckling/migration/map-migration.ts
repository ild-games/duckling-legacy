import {MapVersion, compareVersions, versionCompareFunction, VersionCompatibility} from '../util/version';
/**
 ...
 */
export function migrationsToRun(currentVersion : MapVersion, nextVersion : MapVersion, migrations : MapMigration []) : MapMigration [] {
    return migrations
        .filter((migration) => {
            let alreadyRun = versionCompareFunction(migration.updateTo, currentVersion) <= 0;
            let versionOverflow = versionCompareFunction(nextVersion, migration.updateTo) < 0;
            return !(alreadyRun || versionOverflow);
        }).sort((migrationA, migrationB) => versionCompareFunction(migrationA.updateTo, migrationB.updateTo));
}
