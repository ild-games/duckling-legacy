...
export class CollisionTypesService {
    ...
    async postLoadMapHook(map: RawMapFile): Promise<RawMapFile> {
        let json = await this._jsonLoader.getJsonFromPath(
            this._project.getProjectMetaDataPath("collision-types")
        );
        if (this._tryParseJsonFile(json)) {
            this._registerAnconaCollisionTypes(JSON.parse(json));
        } else {
            this._notifyFileError(
                "Error: collision-types.json not properly formatted!"
            );
        }
    ...
    editCollisionType(oldValue: string, newValue: string) {
        if (!this.collisionTypes.value.has(oldValue)) {
            throw new Error(
                `Attempt to edit collision type that is not registered: ${oldValue}`
            );
        }
        ...
        this._store.dispatch(
            _collisionTypesAction(new Set<string>(newCollisionTypes))
        );
    }
    ...
    removeCollisionType(collisionType: string) {
        if (!this.collisionTypes.value.has(collisionType)) {
            throw new Error(
                `Attempt to delete collision type that is not registered: ${collisionType}`
            );
        }
    ...
    private _registerAnconaCollisionTypes(
        collisionTypeMetaData: CollisionTypeMetaData
    ) {
    ...
    private _notifyUnknownCollisionTypes(unknownCollisionTypes: string[]) {
        for (let type of unknownCollisionTypes) {
            this._snackbar.addSnack(
                `Unknown collision type registered from map: ${type}`
            );
        }
    }
    ...
    private _findUnknownCollisionTypes(map: RawMapFile): string[] {
        let rawMapCollisionTypes = this._getUniqueCollisionTypesInRawMapFile(
            map
        );
    ...
    private _getUniqueCollisionTypesInRawMapFile(map: RawMapFile): string[] {
        if (
            !map.systems["collision"] ||
            !map.systems["collision"]["components"]
        ) {
            return;
        }
        ...
        for (let rawEntityAttribute in collisionAttributes) {
            collisionTypes.add(
                collisionAttributes[rawEntityAttribute]["collisionType"]
            );
        }
        ...
        let json = JSON.stringify(
            {
                collisionTypes: Array.from(
                    this.collisionTypes.getValue().values()
                ),
            },
            null,
            4
        );
        this._jsonLoader.saveJsonToPath(
            this._project.getProjectMetaDataPath("collision-types"),
            json
        );
    }
...
export function collisionTypesReducer(
    state: CollisionTypesState = {},
    action: CollisionTypesAction
) {
