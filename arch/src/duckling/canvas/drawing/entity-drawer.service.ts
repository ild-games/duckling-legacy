...
@Injectable()
export class EntityDrawerService extends BaseAttributeService<
    AttributeDrawer<Attribute>
> {
    /**
    ...
    drawEntitySystem(
        ...
        let oldCache: EntityCacheInternal =
        ...
        if (drawer) {
            ...
            if (drawnConstruct) {
                if (this._layers.isAttributeImplemented(key)) {
                    let layerString = this._layers.getAttributeLayer(
                        entity,
                        key
                    );
    ...
    getEntityDrawable(entity: Entity): DrawnConstruct[] {
        ...
        for (let key in entity) {
            ...
            if (
                this._layers.isAttributeImplemented(key) &&
                !this._layers.isAttributeOnAnActiveLayer(entity, key)
            ) {
                continue;
            }
    ...
    isAttributeVisible(attributeKey: string, entity?: Entity) {
        ...
        if (
            entity &&
            this._layers.isAttributeImplemented(attributeKey) &&
            !this._layers.isAttributeOnAnActiveLayer(entity, attributeKey)
        ) {
            return false;
        }
