...
export class CopyPasteService {
    ...
    constructor(
        ...
        this._store.state.subscribe((state) =>
            this.clipboard.next(state.clipboard)
        );
    }
    ...
    paste(position: Vector): EntityKey[] {
        ...
        for (let entity of clipboardEntities) {
            let newEntity = this._position.setPosition(
                entity.entity,
                vectorRound(
                    vectorAdd(
                        position,
                        vectorSubtract(
                            this._position.getPosition(entity.entity),
                            lowestPoint
                        )
                    )
                )
            ...
            pastedEntities.push(
                this._entitySystem.addNewEntity(newEntity, entity.key, mergeKey)
            );
        }
