...
describe("CollisionTypesService", function() {
        ...
        expect(
            Array.from(
                this.storeService.state
                    .getValue()
                    .collision.collisionTypes.values()
            )
    ...
    it("is aware of new collision types added", function() {
        ...
        expect(
            Array.from(
                this.collisionTypesService.collisionTypes.getValue().values()
            )
        ).to.contain("collision-type1");
    });
    ...
    it("retrieve a collision type of 'none' on an entity that has the default collision", function() {
        ...
        expect(
            this.collisionTypesService.getCollisionTypeForEntity(entity)
        ).to.eql(NONE_COLLISION_TYPE);
    });
    ...
    it("can find the unknown collision types from a given map", function() {
        ...
        let unknownCollisionTypes = (this
            .collisionTypesService as any)._findUnknownCollisionTypes(map);
    ...
    it("can get the unique collision types in a given map", function() {
        ...
        let uniqueCollisionTypes = (this
            .collisionTypesService as any)._getUniqueCollisionTypesInRawMapFile(
            map
        );
